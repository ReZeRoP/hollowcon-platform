...
export const roleSchema = z.enum(["owner", "admin", "finance", "support", "server_operator", "marketing", "auditor"]);
...
export const setupCardSchema = z.object({
  pan: z.string().transform((value) => value.replace(/[\s-]/gu, "")).pipe(z.string().regex(/^\d{16}$/u)),
...
export const setupPanelSchema = z.object({
  ...
  baseUrl: z.url().refine((value) => new URL(value).protocol === "https:", "Panel URL must use HTTPS"),
